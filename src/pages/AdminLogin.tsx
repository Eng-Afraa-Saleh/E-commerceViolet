...
const AdminLogin: React.FC = () => {
  ...
  return (
    <div className="pt-30 pb-24 min-h-screen flex items-center justify-center px-4 bg-slate-50">
      <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-500">
        <div className="p-8 ">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="admin@violet.com"
                  className="w-full bg-slate-50 border border-slate-100 rounded-md py-2 pl-12 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-violet-200 transition-all font-medium"
                  required
                />
              </div>
            </div>
            ...
            <button 
              type="submit"
              className="w-full bg-violet-600 text-white py-2 rounded-md font-black text-sm hover:bg-violet-700 transition-all shadow-xl shadow-violet-100 flex items-center justify-center uppercase tracking-widest group"
            >
              Enter Dashboard <ArrowRight size={18} className="ml-2 group-hover:translate-x-1 transition-transform" />
            </button>
          </form>
          
          <p className="mt-10 text-center text-sm font-bold text-slate-600 tracking-widest">
            email : admin@violet.com & pass : 0000
          </p>
        </div>
      </div>
    </div>
